...
WayPointCircle::WayPointCircle(WayPointItem *center, double radius, bool clockwise, MapGraphicItem *map, QColor color, bool dashed, int width) : QGraphicsEllipseItem(map),
    my_center(center), my_radius(NULL), my_map(map), myColor(color), myClockWise(clockwise), dashed(dashed), width(width)
{
    s_radius = radius;
    connect(center, SIGNAL(localPositionChanged(QPointF, WayPointItem *)), this, SLOT(refreshLocations()));
    //connect(radius, SIGNAL(localPositionChanged(QPointF, WayPointItem *)), this, SLOT(refreshLocations()));
...
WayPointCircle::WayPointCircle(HomeItem *center, double radius, bool clockwise, MapGraphicItem *map, QColor color, bool dashed, int width) : QGraphicsEllipseItem(map),
    my_center(center), my_radius(NULL), my_map(map), myColor(color), myClockWise(clockwise), dashed(dashed), width(width)
{
    s_radius = radius;
    connect(center, SIGNAL(localPositionChanged(QPointF, WayPointItem *)), this, SLOT(refreshLocations()));
...
void WayPointCircle::refreshLocations()
{
    ...
    C = my_map->FromLocalToLatLng(my_center->x(),my_center->y());
    C.SetLat(C.Lat() + s_radius/111133.3333f);

    //float R = my_center->pos().-;

    /*qDebug() << my_center->pos()
             << my_map->FromLatLngToLocal(C).X()
             << my_map->FromLatLngToLocal(C).Y()
             << C.Lat()
             << C.Lng();
             */


    line = QLineF(my_center->pos().x(),my_center->pos().y(),my_center->pos().x(),my_map->FromLatLngToLocal(C).Y());
